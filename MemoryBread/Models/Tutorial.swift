...
struct Tutorial {
    ...
    var infos = [
        Info(title: "4. 엑셀 파일로 암기빵 만들기",
             content: "엑셀 파일로 파일당 한 번에 최대 500개의 암기빵을 만들 수 있습니다. 엑셀파일의 A열에 암기빵의 제목을, B열에 암기빵을 내용을 입력하고 저장하세요. 하나의 행은 한 개의 암기빵을 의미합니다. 엑셀 파일을 구글 드라이브에 업로드 하고, 앱 내에서 구글 드라이브에 로그인하고, 파일을 다운로드 받으세요. 암기빵 목록에서 오른쪽 위의 네모난 버튼을 누르면 구글 드라이브에 로그인할 수 있습니다. 엑셀파일을 사용해 대량의 암기빵을 빠르고 쉽게 만들어보세요!",
             filterIndexes: []
            ),
